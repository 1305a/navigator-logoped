...
import { CARDS_BY_LEVEL, prepositionNounImageTasks } from "@/data/prepositionNounNounTrainer";
...
export default function PrepositionNounNounTrainerPage() {
  const navigate = useNavigate();
  const elapsedSeconds = useElapsedSeconds();

  const [level, setLevel] = useState<1 | 2 | 3>(1);
  const [taskIndex, setTaskIndex] = useState(0);
  const task = prepositionNounImageTasks[taskIndex];
  const activePhrases = task.phrases.slice(0, CARDS_BY_LEVEL[level]);

  const prepBankOrder = useMemo(() => shuffleArray(task.prepositions), [task]);
  ...
  const isLastTask = taskIndex === prepositionNounImageTasks.length - 1;
  const isLastLevel = level === 3;
  const isTaskComplete = activePhrases.every((p) => !!filled[p.id]);
  const fieldLocked = isTaskComplete;
  const availablePrepositions = prepBankOrder.filter((p) => !usedPrepositions.has(normPrep(p)));
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-6">
      <div>
        ...
        <h1 className="text-2xl font-semibold text-foreground">
          Расставьте предлоги (существительное + существительное)
        </h1>
        ...
        controls={
          <>
            ...
            <Select value={String(taskIndex)} onValueChange={(v) => v && setTaskIndex(Number(v))}>
              ...
              <SelectContent>
                {prepositionNounImageTasks.map((_, idx) => (
            ...
            <span className="text-sm text-muted-foreground">из {prepositionNounImageTasks.length}</span>
          </>
        }
