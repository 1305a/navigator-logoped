...
import { categoryObjectLevels } from "@/data/categoryObjectsTrainer";
...
import type { GenderItem } from "@/data/genderMasculineTrainer";

const MAX_WRONG = 3;

export default function CategoryObjectsTrainerPage() {
  const navigate = useNavigate();
  const elapsedSeconds = useElapsedSeconds();

  const [levelIndex, setLevelIndex] = useState(0);
  const level = categoryObjectLevels[levelIndex];
  ...
  useEffect(() => {
    setGrid(shuffleArray(level.items));
  ...
  }, [taskIndex, levelIndex, level.items]);

  const isLastTask = taskIndex === level.tasks.length - 1;
  const isLastLevel = levelIndex === categoryObjectLevels.length - 1;
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-6">
      <div>
        ...
        <h1 className="text-2xl font-semibold text-foreground">Выбор предмета Категории 1-5</h1>
        <p className="text-sm text-muted-foreground">
          Пять уровней по темам; нажмите «Послушайте» и выберите картинку
        </p>
      </div>
        ...
        controls={
          <>
            ...
            <Select
              ...
              onValueChange={(v) => v && !audioLocked && !feedback && handleLevelChange(Number(v))}
            >
              <SelectTrigger className="w-44">
                <SelectValue>{() => level.title}</SelectValue>
              </SelectTrigger>
              <SelectContent>
                {categoryObjectLevels.map((l, idx) => (
      ...
      {readyToProceed && (
        <TrainerAdvanceButton
          isLast={isLastTask && isLastLevel}
