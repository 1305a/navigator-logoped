...
export default function VerbWordsTrainerPage() {
  const navigate = useNavigate();
  const elapsedSeconds = useElapsedSeconds();

  const [exerciseIndex, setExerciseIndex] = useState(0);
  const exercise = verbWordsExercises[exerciseIndex];
  ...
  const isLast = exerciseIndex === verbWordsExercises.length - 1;
    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      <div>
        ...
        <h1 className="text-2xl font-semibold text-foreground">
          Слова-действия и есть глаголы
        </h1>
        <p className="text-sm text-muted-foreground">Подберите слова-действия к глаголам</p>
      </div>
      ...
      <TrainerStatsBar
        controls={
          <>
            ...
            <span className="text-sm text-muted-foreground">из {verbWordsExercises.length}</span>
          </>
        }
